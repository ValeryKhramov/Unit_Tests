...
*/
    ...
    void whenChangingCartCostRecalculationIsCalled() {
        ...
        assertThat(cart.getTotalPrice()).isEqualTo(170 + 250 + 200 - 200);
    }
    ...
    void quantityProductsStoreChanging() {
        ...
        cart.addProductToCartByID(1);
        cart.addProductToCartByID(1);
        int finalQuan = shop.getProductsShop().get(0).getQuantity();
        assertThat(finalQuan).isEqualTo(startQuan - 3);
...
/*        for (int i = 0; i < 11; i++) {
    ...
    void deletedProductIsReturnedToShop() {
        ...
        cart.addProductToCartByID(1);
        cart.addProductToCartByID(1);
        int countCartQuantity = shop.getProductsShop().get(0).getQuantity();

        cart.removeProductByID(1);
        assertEquals(countCartQuantity + 1, shop.getProductsShop().get(0).getQuantity());
     ...
     */
    @ParameterizedTest // У теста есть параметр
    @ValueSource(ints = {0, 13})
    // Перечисление граничных условий
    void incorrectProductSelectionCausesException(int param) {
        assertThatThrownBy(() -> cart.addProductToCartByID(param)).
    ...
    void incorrectProductRemoveCausesException() {
        RuntimeException exc = assertThrows(RuntimeException.class, () -> cart.removeProductByID(1));
        // Ловим исключение и можем его в дальнейшем использовать
        assertThat(exc.getMessage()).isEqualTo("В корзине не найден продукт с id: 1"); // Первый способ

        assertThatThrownBy(() -> cart.removeProductByID(1)).
                isInstanceOf(RuntimeException.class).
                hasMessage("В корзине не найден продукт с id: 1"); // Второй способ
    }
